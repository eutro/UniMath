...
(** [UU_rxgraph] is the (large) reflexive graph of (small) types.
    It is univalent precisely if the universe is. *)
...
Notation "A '×!' B" := (dirprod_rxgraph' A B) (at level 75, right associativity) : rxgraph_spec.
(* type in Emacs using agda-input with \times *)

Notation "A ⨿ B" := (coprod_rxgraph A B) : rxgraph_spec.
Notation "A '⨿!' B" := (coprod_rxgraph' A B) (at level 75, right associativity) : rxgraph_spec.
(* type in Emacs with C-X 8 RET AMALGAMATION OR COPRODUCT
   or using agda-input with \union (on the second page). *)

Notation "A '^op'" := (opp_rxgraph A) (at level 1, format "A ^op") : rxgraph_spec.
Notation "A '^op!'" := (opp_rxgraph' A) (at level 1, format "A ^op!") : rxgraph_spec.

Notation "E ⟦ x ⟧" := (disp_rxgraph_at E x) (at level 49) : rxgraph_spec.
Notation "E ⟦ x '⟧!'" := (disp_rxgraph_at' E x) (at level 49) : rxgraph_spec.
(* type in Emacs using agda-input with \[[ \]] *)

Notation "'{'  x '∣' P  '}'" := (sub_rxgraph _ (λ x, P)) (x binder) : rxgraph_spec.
Notation "'{'  x '∣![' H  ']' P  '}'" := (sub_rxgraph_pred' _ (λ x, P) (λ x, H)) (x binder) : rxgraph_spec.
Notation "'{'  x '∣!' P  '}'" := (sub_rxgraph' _ (λ x, P)) (x binder) : rxgraph_spec.
